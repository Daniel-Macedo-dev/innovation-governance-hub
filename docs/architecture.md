...
Serviços de domínio escrevem eventos pelo `AuditService`; páginas Streamlit não criam eventos diretamente. `AuditEvent` é append-only na operação normal. `AIGovernanceDecision` preserva transições decisórias, `ImportBatch` impede repetição pelo SHA-256 e `NotificationService` controla alertas. A projeção financeira é determinística e usa `Decimal`.
