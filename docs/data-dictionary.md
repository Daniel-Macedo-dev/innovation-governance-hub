...
`AuditEvent` registra ator, ação, resumo e mudanças sanitizadas. `AIGovernanceDecision` mantém decisões sem sobrescrever o passado. `ImportBatch` registra fingerprint, tipo, quantidade e responsável. `NotificationLog.lifecycle_status` usa Novo, Reconhecido, Resolvido ou Ignorado. JSON de auditoria não armazena tokens, senhas, chaves ou conteúdo integral de documentos.
